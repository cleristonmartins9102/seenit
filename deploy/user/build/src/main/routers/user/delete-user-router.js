"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.deleteUserRouter = void 0;
const adapters_1 = require("../../../infra/adapters");
const delete_user_controller_factory_1 = require("../../../main/factory/controller/delete-user-controller-factory");
const middlewares_1 = require("../../../main/middlewares");
const permission_middleware_1 = require("../../../main/middlewares/permission-middleware");
const deleteUserRouter = (router) => {
    const appSecurity = {
        app: {
            name: 'user',
            id: '1',
            module: [{
                    name: 'delete',
                    id: '3',
                    accepted: permission_middleware_1.Permissions.delete
                }]
        }
    };
    router.delete('/user/delete', middlewares_1.authMiddleware, (0, permission_middleware_1.permissionMiddeware)(appSecurity), (0, adapters_1.expressAdapter)((0, delete_user_controller_factory_1.deleteUserControllerFactory)()));
};
exports.deleteUserRouter = deleteUserRouter;
